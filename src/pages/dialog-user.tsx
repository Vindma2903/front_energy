 ...
 const handleSelectChat = async (id: string, userName: string) => {
  ...
  const handleSendMessage = async () => {
    ...
    const newMessage = {
      ...
      text: message,
      sender: currentUserId, // ✅ Здесь должен быть ID текущего пользователя
      role: currentUserRole, // ✅ Используем роль (LEAD, BOT, MANAGER)
    };
    ...
    try {
      ...
      // Добавляем сообщение в локальный список (без перезагрузки)
      const currentTime = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
      setChatMessages((prev) => [
        ...prev,
        { from: newMessage.role.toLowerCase(), text: message, time: currentTime },
      ]);
            ...
            {chatMessages.map((msg, index) => (
              <div key={index} className="nav-msg-user-chat-message-wrapper">
                <div className="nav-msg-user-message-avatar">
                  <img
                    src="/public/free-icon-woman-avatar-17932080.png"
                    alt="Аватар"
